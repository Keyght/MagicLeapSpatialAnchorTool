...
public class SpatialAnchorsTool : MonoBehaviour
{
    ...
    public void DeleteAllAnchors()
    {
        ...
        {
            storageFeature.DeleteStoredSpatialAnchors(storedAnchors);
            Destroy(anchor.gameObject);
    ...
    {
        if (!storageFeature.QueryStoredSpatialAnchors(controllerObject.transform.position, 50f))
        {
    ...
    void Update()
    {
        if (permissionGranted)
        {
            if (localizationMapFeature != null)
            {
                ...
                deleteButton.interactable = mapData.State == LocalizationMapState.Localized;
            }
            else
            {
